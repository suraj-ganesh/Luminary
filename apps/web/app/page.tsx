...
export default function Home() {
  ...
  // Inline Playground component for the Before/After demo
  const Playground = () => {
    ...
    const samples: Record<'contrast' | 'alt' | 'keyboard', { before: string; after: string; explain: string }> = {
      contrast: {
        ...
        explain: 'Adjusted colors to meet WCAG contrast ratios and improved label clarity.'
      },
      alt: {
        before: `<img src="/product.jpg" />`,
        after: `<img src="/product.jpg" alt="Close-up of the Luminary dashboard showing color contrast improvements" />`,
        explain: 'Added descriptive alt text generated by neural vision to aid screen reader users.'
      },
      keyboard: {
        before: `<div onclick="open()" style="cursor:pointer;">Open</div>`,
        after: `<button type="button">Open</button>`,
        explain: 'Replaced non-semantic clickable div with a focusable, keyboard-accessible button element.'
      }
    };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <button onClick={() => setTab('contrast')} className={`px-3 py-2 rounded-md text-[12px] font-bold ${tab === 'contrast' ? 'bg-black text-white' : 'bg-white/40'}`}>Color Contrast</button>
          <button onClick={() => setTab('alt')} className={`px-3 py-2 rounded-md text-[12px] font-bold ${tab === 'alt' ? 'bg-black text-white' : 'bg-white/40'}`}>Alt Text</button>
          <button onClick={() => setTab('keyboard')} className={`px-3 py-2 rounded-md text-[12px] font-bold ${tab === 'keyboard' ? 'bg-black text-white' : 'bg-white/40'}`}>Keyboard</button>
          <div className="ml-auto flex items-center gap-2">
            <span className="text-[11px] text-muted-foreground">View</span>
            <button onClick={() => setShowAfter(!showAfter)} className="px-3 py-1 rounded-lg bg-white/10 text-white text-[12px]">{showAfter ? 'Patched' : 'Original'}</button>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-black/5 rounded-lg">
            <div className="text-[10px] font-bold uppercase tracking-widest mb-2">Original</div>
            <pre className="text-xs overflow-auto p-2 rounded bg-white/70 border border-black/5">{current.before}</pre>
          </div>
        ...
        <div className="p-4 rounded-lg bg-white/50 border border-black/5">
          <div className="text-sm font-bold mb-1">Explanation</div>
          <div className="text-sm text-muted-foreground">{current.explain}</div>
        </div>
      </div>
    );
  };
      ...
      <main className="flex-1 flex flex-col pt-32 pb-40 w-full overflow-hidden">
        ...
        {!user && (
          <>
            ...
            <section id="how-it-works" className="w-full py-20 flex justify-center px-6 overflow-hidden">
              <div className="container max-w-6xl space-y-10">
                <motion.div {...fadeInUp} className="text-center space-y-4">
                  <h2 className="text-4xl font-light tracking-tight">How It Works</h2>
                  <p className="text-muted-foreground max-w-2xl mx-auto">A visual walkthrough of Luminary's automated crawl, neural analysis, and safe code remediation pipeline.</p>
                </motion.div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    {
                      step: 'Step 01',
                      title: 'Deep Visual Crawl',
                      desc: 'Spawns headless Chromium instances to execute custom actions and fully render dynamic content.'
                    },
                    {
                      step: 'Step 02',
                      title: 'Neural Analysis',
                      desc: 'Cross-references AxE-core accessibility rules with LLM synthesis to eliminate false positives.'
                    },
                    {
                      step: 'Step 03',
                      title: 'Code Auto-Remediation',
                      desc: 'Translates violations into functional, side-effect-free, WCAG-compliant React/HTML patches.'
                    }
                  ].map((s, i) => (
                    <motion.div key={s.step} {...fadeInUp} transition={{ delay: i * 0.12 }} className="p-8 rounded-3xl bg-white/40 backdrop-blur-md border border-white/10 hover:scale-105 transition-transform shadow-2xl">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div className="h-12 w-12 rounded-xl bg-gradient-to-tr from-[#3b83f5]/20 to-[#2ecac5]/20 flex items-center justify-center text-white font-bold">{s.step.split(' ')[1]}</div>
                          <div>
                            <h4 className="text-lg font-bold">{s.title}</h4>
                            <p className="text-[12px] text-muted-foreground max-w-xs">{s.desc}</p>
                          </div>
                        </div>
                        <div className="h-10 w-10 rounded-lg bg-white/20 flex items-center justify-center text-black/30">⚡</div>
                      </div>
                      <div className="text-[13px] text-muted-foreground mt-2">Hover to preview details.</div>
                    </motion.div>
                  ))}
                </div>
              </div>
            ...
            {/* Code Playground - Before & After */}
            <section id="playground" className="w-full py-20 flex justify-center px-6 overflow-hidden">
              <div className="container max-w-6xl space-y-8">
                <motion.div {...fadeInUp} className="flex items-center justify-between">
                  <div>
                    <h3 className="text-3xl font-light">Before & After Playground</h3>
                    <p className="text-muted-foreground text-sm">Toggle common accessibility issues and inspect the AI-generated fixes.</p>
                  </div>
                </motion.div>

                <motion.div {...fadeInUp} className="bg-white/90 rounded-3xl p-6 border border-black/5 shadow-lg">
                  <Playground />
                </motion.div>
              </div>
            </section>
            ...
            {/* Guest CTA */}
            <section id="guest-cta" className="w-full py-16 flex justify-center px-6">
              <motion.div {...fadeInUp} className="container max-w-5xl bg-gradient-to-br from-black/80 to-[#0f172a]/80 rounded-3xl p-10 text-white shadow-2xl relative overflow-hidden">
                <div className="absolute inset-0 pointer-events-none opacity-30 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#3b83f5] to-[#2ecac5]"></div>
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div>
                    <h3 className="text-3xl font-bold">Ready to audit your projects?</h3>
                    <p className="text-muted-foreground text-sm mt-2 text-white/80">Sign up and run a full site audit with AI-driven fixes and explanations.</p>
                  </div>
                  <div className="flex items-center gap-4">
                    <Link href="/signup" className="px-6 py-3 bg-white text-black rounded-full font-bold hover:scale-105 transition-transform shadow-lg">Get Started</Link>
                    <Link href="/pricing" className="px-4 py-3 border border-white/20 rounded-full text-white/90 text-[13px] font-bold hover:bg-white/5 transition-colors">See Pricing</Link>
                  </div>
                </div>
              </motion.div>
            </section>
